...
int AVLT :: max(int x, int y){ // Simple maximum function
  if(x > y){
    return x;
  }
  return y;
...
Node *AVLT :: insert(Node* start, Node* to_insert) //Base code taken from Professor's code
{
  ...
  else if(to_insert->key > start->key){ //Go right if larger
    //cout << "Right Insert \n";
  ...
  if(balance < -1 && to_insert->key > start->right->key){ //Balance < 1 means left is smaller than right, this is the right right case because the new node was inserted right of right child
    //cout << "Right Right\n" << flush;
	...
	else if(balance > 1 && to_insert->key < start->left->key){ //Balance > 1 means left side is larger than right side, this is left left case because the new node key is less than the left chi
    //cout << "Left Left\n" << flush; 
  ...
  else if(balance < -1 && to_insert->key < start->right->key){ //This is the right left case, the new node was inserted under the right child, but left because it was smaller than the right key
    //cout << "Right Left\n" << flush;
  ...
  else if(balance > 1 && to_insert->key > start->left->key){ //This is left right case because the the inserted node was smaller than parent but the inserted node is larger than the left child
    //cout << "Left Right\n" << flush;
...
Node *AVLT :: rightRotate(Node *start){
  ...
  Node *leftRight = leftChild->right; //Right child of left child of start
   ...
   if(start->parent != NULL){ //Check for parent
  ...
  if(originalParent != NULL){//Do if there was a parent of start 
...
Node *AVLT :: leftRotate(Node *start){
  ...
  if(start->parent != NULL){ //If start has a parent, save that in original parent
  ...
  if(rightLeft != NULL){ //If its not null 
...
void AVLT :: increaseSubTree(Node *start){ //This function increases the subtree size of all parent nodes starting from start
  if(start == NULL){ //Use only after inserting, reason this is a seperate method because this needs to increase subtree up to root,
...
int AVLT :: range(Node *start, string str1, string str2){
  ...
  if(start == NULL){ //Most basic case, dont recurse anymore if at leaf
    return 0; 
  }
	if(start->key < str1 || start->key > str2){ //Remove this node if its not in range, could be leaf or intermediate node
    toRemove++;
  }
	
	
  if(start->key <= str1){ //Second base case, if the node is lower to str1, everything smaller than it wont count
    if(start->left != NULL){
  ...
  else if(start->key >= str2){ //Third base case, if the node is larger  str2, everything larger than it wont count
    if(start->right != NULL){
...
int AVLT :: findLeft(Node *start, string str1){
	...
	if (start == NULL){ //First base case
		return 0;
	}
	if(start->left == NULL && start->right == NULL && start->key < str1){ //Second base case
		return 1;
	}
	
	if(start->key == str1){ //Third base case
		if(start->left != NULL){
	...
	if(start->key < str1){ //First time value of node is less than lower bound
		toRemove = 1; //Gotta add this to the number of out of bound lists
		if(start->left != NULL){ //If there is a left child 
			if(start->right != NULL){ //If there is a right child
    ...
    if(start->right != NULL){ //If no left child but yes right child, will have to iterate through to see if there are any out of bounds
      return(toRemove + findLeft(start->right, str1));
    }
	}
	
	if(start->key > str1){ //Since current node is in bounds, keep going left
...
int AVLT :: findRight(Node  *start, string str1){
	int toRemove = 0;
	if (start == NULL){ //One Base case
		return 0;
	}
	if(start->right == NULL && start->left == NULL && start->key > str1){ //Second base case
		return 1;
	}
	if(start->key == str1){ //Third base case
		if(start->right != NULL){
	...
	if(start->key > str1){ //This is supposed to be the first node where value is higher than the current bound
		toRemove = 1; //This node is over so it should also be removed
		if(start->right != NULL){ //If there is right child
			if(start->left != NULL){
    ...
    if(start->left != NULL){ //This is if there is no right child but there is left which can be in bounds
      return(toRemove + findRight(start->left, str1));
    }
	}
	
	if(start->key < str1){ //Since current node value is still lower than the higher bound, keep going right
...
void AVLT :: setHeight(Node *start){
  ...
  else if(start->left == NULL){ //Height logic, height of highest child + 1
...
void AVLT :: printTree(Node *start, int space){ //Print debugging code taken from GFG
  if(start == NULL){
    return;
  }
  space += 4;
