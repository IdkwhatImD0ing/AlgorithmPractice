...
class LogisticRegressionTrainer:
    ...
    def __init__(
        ...
        # Initialize weights for your model.
    ...
    def gradient_descent_step(self, x: np.ndarray, y: np.ndarray) -> None:
        ...
        self.theta = self.theta - (alpha *
                                   self.cross_entropy_loss_derivative(x, y))
    ...
    def cross_entropy_loss(self, pred: np.ndarray,
        ...
        #print(pred)
        #print(target)
        answer = (-1 * 1 / m * np.sum(target * np.log(pred) +
                                      (1 - target) * np.log(1 - pred))
                  ) + (self.lambd * np.sum(self.theta[1:]**2))
        #print(answer)
        return (answer)

    def cross_entropy_loss_derivative(self, x: np.ndarray,
        ...
        answer = (1 / m * (np.sum(
            (pred - y)[:, None] * x, axis=0))) + (self.lambd *
                                                  np.sum(2 * self.theta[1:]))
        #print(answer)
        return (answer)
    ...
    def train(
        ...
        for i in range(self.num_epochs):
            ...
            self.train_loss_history.append(
                self.cross_entropy_loss(hypo_train, y_train))
            self.val_loss_history.append(
                self.cross_entropy_loss(hypo_val, y_val))
